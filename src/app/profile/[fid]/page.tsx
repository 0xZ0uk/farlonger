...
export default function ProfileByFID({ params }: { params: { fid: string } }) {
  const { data } = useSWR(`/api/hub/following?fid=${params.fid}`, fetcher);

  const { data: user, isLoading: isUserLoading } =
    api.user.getUserByFID.useQuery({
      fid: Number(params.fid) ?? 1,
    });

  const { data: posts, isLoading } = api.post.getByUserFID.useQuery({
    fid: params.fid ?? 1,
  });
    ...
    <main className="flex min-h-[calc(100vh-9rem-1px)] items-start justify-between p-8 pt-28 md:px-12 lg:px-24">
      <div className="absolute left-8 flex w-[calc(100%-4rem)] flex-col items-center justify-center gap-4 sm:left-12 md:w-[calc(100%-24rem-7rem)] lg:left-24 lg:w-[calc(100%-24rem-13rem)]">
        <div className="mt-12 w-full">
          {!!user ? (
            <h1 className="mb-8 text-6xl font-bold">{user?.username}</h1>
          ) : (
            <Skeleton className="mb-8 h-20 w-64 font-bold" />
          )}
          <Posts posts={posts ?? []} loading={isLoading ?? false} />
        </div>
      </div>
